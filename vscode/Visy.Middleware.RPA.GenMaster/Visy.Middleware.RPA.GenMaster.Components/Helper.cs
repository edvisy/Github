...
namespace Visy.Middleware.RPA.GenMaster.Components
{
    public class Helper
    {
        private const string INBOX_INTERFACE_NAME = "RPA.GenMaster.InboxName";
        ...
        public static string GetMessageId(string str)
        {
            ...
            if (match.Success)
            {
                System.Diagnostics.EventLog.WriteEntry("BizTalkApp", "RPA.GenMaster: Message-Id" + match.Groups[1].Value);
        ...
        public static string GetForwardingEmail(string str)
        {
            var match = Regex.Match(str, @"Message-ID: <(.*)>");
            if (match.Success)
            {
