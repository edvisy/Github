...
namespace Visy.Middleware.SAP.JFE.Components
{
    public class GoodsReceiptBuilder: BaseComponent
    {
        ...
        private List<GoodsReceiptItem> PopulateItems(Order order,string visyPO )
        {
            ...
            foreach(OrderOrderLine line in order.OrderLine)
            {
                System.Diagnostics.Trace.WriteLine("DEBUG inside populateitem loop current VisyPONumber " + GetPO(line.VisyPONumber).Trim());
                GoodsReceiptItemBatch batch = new GoodsReceiptItemBatch();

                if (GetPO(line.VisyPONumber).Trim() == visyPO.Trim())
                {
                    System.Diagnostics.Trace.WriteLine("SDEBUG inside populateitem SUCCESS");
                    batch.CoilNumber = line.RollNumber;
                    batch.ItemNumber = line.ProductCode;
                    batch.Quantity = line.NetWeight;
                    batch.UOM = line.UOM;
                    batch.POSEX = line.ItemNumber;
                    batch.Invoice = line.Invoice;
                    batch.BillOfLading = line.BillOfLading;
                    batches.Add(batch);
                }
            }
        ...
        private GoodsReceiptHeader PopulateHeader(Order order,string visyPO)
        {
            ...
            foreach (OrderOrderLine line in order.OrderLine)
            {
                if (GetPO(line.VisyPONumber).Trim() == visyPO.Trim())
                {
                    headerDetail.Name = "TOYOTA TSUSHO AUSTRALASIA P/L";
                    headerDetail.DeliveryDate = line.ShippingDateFromWorks; //SetDeliveryDate(order.OrderLine[0].ShippingDateFromWorks);
                    headerDetail.PlantCode = "2105";
                    headerDetail.PONumber = visyPO; //GetPO(order.OrderLine[0].VisyPONumber);
                    headerDetail.VendorNumber = "110544";
                    headerDetail.VoyageNumber = line.VoyageNumber; //order.OrderLine[0].VoyageNumber;
                    break;
                }
            }
            return headerDetail;
