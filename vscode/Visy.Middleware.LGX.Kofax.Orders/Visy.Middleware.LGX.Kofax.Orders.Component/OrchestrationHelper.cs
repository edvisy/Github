...
namespace Visy.Middleware.LGX.Kofax.Orders.Component
{
    ...
    public class OrchestrationHelper
    {
        ...
        public static string Base64ToXml(XLANGMessage base64)
        {
            ...
            nsmgr.AddNamespace("ns0", "http://Visy.Middleware.LGX.Kofax.Orders.Schemas.CorrelationWrapper");
        ...
        public static string GetNotificationRecipient(string filename) {
            Regex rx = new Regex(@"(.*?)~");
            var str = Visy.Middleware.Components.Utilities.DataLookupHelper.GetInterfaceLookupData(rx.Match(filename).Groups[1].Value, "LGX.Kofax.Orders.EmailGroup.P1");
            
            if (str == rx.Match(filename).Groups[1].Value)
                str =  Visy.Middleware.Components.Utilities.DataLookupHelper.GetInterfaceLookupData((rx.Match(filename).Groups[1].Value).Split('_')[0] + '_', "LGX.Kofax.Orders.EmailGroup.P2");
           
            if (str == (rx.Match(filename).Groups[1].Value).Split('_')[0] + '_')
                return Visy.Middleware.Components.Utilities.DataLookupHelper.GetInterfaceLookupData("biztalk_team", "LGX.Kofax.Orders.EmailGroup.P2"); ;
