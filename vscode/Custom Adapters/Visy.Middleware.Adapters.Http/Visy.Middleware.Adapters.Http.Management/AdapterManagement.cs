//---------------------------------------------------------------------
// File: Visy.Middleware.Adapters.Http.Management.cs
// 
...
namespace Visy.Middleware.Adapters.Http.Management
{
    /// <summary>
    /// Class StaticVisy.Middleware.Adapters.Http.Management implements
    /// IAdapterConfig and IStaticAdapterConfig interfaces for
    /// management to illustrate a static adapter that uses the
    /// adapter framework
    /// </summary>
    public class HttpAdapter : IAdapterConfig, IStaticAdapterConfig, IAdapterConfigValidation 
	{
		private static ResourceManager resourceManager = new ResourceManager("Visy.Middleware.Adapters.Http.Management.HttpAdapterResource", Assembly.GetExecutingAssembly());
        ...
        public string GetConfigSchema(ConfigType type) 
		{
            switch (type) 
			{
				case ConfigType.ReceiveHandler:
					return LocalizeSchema(GetResource("Visy.Middleware.Adapters.Http.Management.ReceiveHandler.xsd"), resourceManager);

				case ConfigType.ReceiveLocation:
					return LocalizeSchema(GetResource("Visy.Middleware.Adapters.Http.Management.ReceiveLocation.xsd"), resourceManager);
	            
				case ConfigType.TransmitHandler:
					return LocalizeSchema(GetResource("Visy.Middleware.Adapters.Http.Management.TransmitHandler.xsd"), resourceManager);

				case ConfigType.TransmitLocation:
					return LocalizeSchema(GetResource("Visy.Middleware.Adapters.Http.Management.TransmitLocation.xsd"), resourceManager);
        ...
        public string[] GetServiceDescription(string[] wsdls) 
		{
            ...
            result[0] = GetResource("Visy.Middleware.Adapters.Http.Management.service1.wsdl");
            return result;
        }
		...
		{
            string result = GetResource("Visy.Middleware.Adapters.Http.Management.CategorySchema.xml");
            return result;
        }
