...
namespace Visy.Middleware.SAP.Kemeny.Components
{
        ...
        private List<SinglePOASNDetail> PopulateItems(MultiPOASN multiPOASN, string visyPO)
        {
            ...
            foreach (MultiPOASNDetail line in multiPOASN.Detail)
            {
                ...
                if (GetPO(line.VisyPO).Trim() == visyPO.Trim())
                {
                    ...
                    batch.ETA = line.ETA;
                    batch.HeatNo = line.HeatNo;
                    batch.KOPCNo = line.KOPCNo;
                    batch.KOPCInvoiceNo = line.KOPCInvoiceNo;
                    batch.NetWeight = line.NetWeight;
                    batch.ProductCode = line.ProductCode;
                    batch.RollNo = line.RollNo;
                    batch.ShippingDate = line.ShippingDate;
                    batch.Size = line.Size;
                    batch.Spec = line.Spec;
                    batch.UOM = line.UOM;
                    batch.VesselVoyage = line.VesselVoyage;
                    batch.Coating = line.Coating;
                    batches.Add(batch);
                }
            }
